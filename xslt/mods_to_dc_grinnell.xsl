...
<xsl:stylesheet version="1.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  <!-- This is mods_to_dc_grinnell.xsl
       Lifted from mods_to_dc_grinnell-modified-2015-May-06.xsl
       Last modified 26-August-2015 -->

  <!-- The following function will not validate!
  xmlns:grinnell="http://digital.grinnell.edu" exclude-result-prefixes="mods srw_dc grinnell">

  <xsl:function name="grinnell:titleCase" as="xs:string">
    <xsl:param name="str" as="xs:string" />
    <xsl:sequence select="string-join(for $x in tokenize($str,'\s') return concat(upper-case(substring($x, 1, 1)), lower-case(substring($x, 2))), ' ')"/>
  </xsl:function>
  -->
  ...
  <!-- MAM adding this template to pick up and distinguish /mods:titleInfo[@type='alternative']
       from /mods:titleInfo (the main title)  -->
  ...
  <xsl:template match="mods:name">
    <xsl:if test="mods:namePart[normalize-space()]">
      <xsl:choose>
        <xsl:when
            test="mods:role/mods:roleTerm[@type='text']='creator' or
                  mods:role/mods:roleTerm[@type='text']='interviewee' or
                  mods:role/mods:roleTerm[@type='text']='artist' or
                  mods:role/mods:roleTerm[@type='text']='author' or
                  mods:role/mods:roleTerm[@type='code']='cre' ">
          <dc:creator>
  ...
  <!-- OK, this next section is critical...it should only select MODS 'note'
       values that are subordinate to the mods root.  Why?  Because we should NEVER export
       mods:note values which are wrapped by a <relatedItem type='admin'> parent. -->

  <!-- MAM edits from 17-August-2016.  Actually, I don't think we need to push any
        /mods/note fields into dc:description!  They just clutter up what needs to be displayed
        there, and are always presented, when appropriate in our MODS metadata display.
        This next block of XSL is therefore commented out.

	<xsl:template match="/mods:mods/mods:note[not(@displayLabel='Import Index')]">
	  <dc:description>
            <xsl:value-of select="concat( '; ', @type, ': ', . )"/>
  	  </dc:description>
	</xsl:template>        -->
  ...
  <!-- More MAM edits from 17-August-2016.  It looks like too many dates are being pushed from
        MODS originInfo/date* fields into dc:date.  Since dc:date drives our Solr faceting
        I don't think this is wise so I have taken steps, in the active block below, to push
         ONLY the /mods/originInfo/dateCreated field into dc:date.
  ...
  What follows below is the XSL implemented on 17-August-2016 to replace the original
  block above.  This active XSL assumes that /mods/originInfo/dateCreated will be a properly
  formatted, discrete date.  It may be in YYYY form, YYYY-mm, YYYY-mm-dd or something more
  granular (with a specific time). 	-->
  ...
  <!-- End of dateCreated -> dc:date active transform -->
  ...
  <!-- MAM adding a match template to pick up /mods:genre and translate these
       to DC as /dc.type with a prefix of 'Genre:' prefixed to the body of the value. -->
  ...
  <!-- MAM added @displayLabel logic to template below.  Split one template into three different templates.

    <xsl:template match="mods:physicalDescription[not(@*)]">
  ...
  <xsl:template match="mods:physicalDescription">
    <xsl:for-each select="mods:extent | mods:form | mods:internetMediaType">
      <dc:format>
        <xsl:value-of select="../@displayLabel"/> <xsl:text> </xsl:text> <xsl:value-of select="local-name()"/> = <xsl:value-of select="."/>
      </dc:format>
    </xsl:for-each>
  </xsl:template>
  ...
  <xsl:template match="mods:language">
    <xsl:variable name="text" select="normalize-space(mods:languageTerm[@type='text'])" />
    <xsl:variable name="code" select="normalize-space(mods:languageTerm[@type='code'])" />
    <xsl:choose>
      <xsl:when test="$text">
        <dc:language>
  ...
  <!-- What follows is Islandora 7's original MODS->DC logic for mods:relatedItem translation.
				...
				<dc:source>
					<xsl:for-each
						...
						<xsl:if test="normalize-space(.)!= ''">
							<xsl:value-of select="."/>
							<xsl:if test="position()!=last()">xx</xsl:if>     Note: Double-dashes here replaced with 'xx' to enable commenting!
						</xsl:if>
					</xsl:for-each>
				</dc:source>
			</xsl:when>
  ...
  -->
  <!-- What follows is my (MAM) modified MODS->DC logic for mods:relatedItem translation -->
  ...
  <xsl:template match="mods:relatedItem[mods:titleInfo|mods:name|mods:identifier|mods:location]">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <dc:relation>
          <xsl:for-each
              select="mods:titleInfo/mods:title">
            <xsl:if test="normalize-space(.)!= ''">
              <xsl:value-of select="concat( ../../@type, ': ', . )"/>
              <xsl:if test="position()!=last()">--</xsl:if>
            </xsl:if>
  ...
  <!-- What follows is my (MAM) MODS->DC logic for mods:relatedItem[@displayLabel="Transcribe This Item"
         This MODS tag needs to yield a simple dc:relation tag with a value of "transcribe".  -->

  <xsl:template match="mods:relatedItem[@displayLabel='Transcribe This Item']">
    <dc:relation>transcribe</dc:relation>
  </xsl:template>
  ...
  <xsl:template name="name">
    <xsl:variable name="name">
      <!-- is there an AACR2 version? -->
      <xsl:choose>
        <xsl:when test="mods:namePart[not(@type) and normalize-space()]">
          <xsl:value-of select="mods:namePart[not(@type) and normalize-space()]"/>
        </xsl:when>
        <xsl:otherwise>
  ...
  <!-- MAM - These parts commented out on 30-July-2014 because they are NOT part of the
       new 'standard' MODS-to-DC transform.
